...
//                .config("hive.metastore.uris", "jdbc:mysql://localhost:3306/metastore?serverTimezone=UTC&zeroDateTimeBehavior=CONVERT_TO_NULL")
    ...
    private MysqlToDirConfig config;

    public MysqlToDir(MysqlToDirConfig config) {
        this.config = config;
    }
    ...
    public void exportDataToTmp() throws IOException, InterruptedException {
        ...
        Path tmpDir = new Path(config.getPathDirTo());
        if(fs.exists(tmpDir)){
        ...
        String command = String.format("sqoop import " +
                ...
                ,config.getHostDB()
                ,config.getPortDB()
                ,config.getFromDbName()
                ,config.getUsernameDB()
    ...
    public static void main(String[] args) throws Exception {
        new MysqlToDir(MysqlToDirConfig.getConfig(args))
                .exportDataToTmp();
    }
